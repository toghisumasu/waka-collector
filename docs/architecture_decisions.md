...
### D-61-1　七句去物ルールの実装・本番適用：成功、ng率上昇は新ルールの正常な発火

**判断：** D-58-1で見送っていた七句去物ルール（`ShikimokuChecker#nanaku_gomono_violations`、
`app/data/nanaku_gomono_words.yml`10語：衣・竹・田・船・夢・涙・月・松・枕・煙）を
実装し、`all_violations`経由で本番適用した。D-59-1（ichiza配線バグ修正）により
`:word`配線ギャップが解消済みだったため、`ichiza_violations`と同様`:text`優先・
`:word`フォールバック方式で実装し、同じ配線ギャップを継承しないようにした。
`verify_shikimoku.rb`試験14（9チェック）追加、全pass（総合98 pass / 0 fail）。

**本番100句run（`sono61_run1`）結果：** 完走（verse_no:100、終了コード0、
action:"error" 0件）。ng率54%（其の四十五 Run1〜4の参考レンジ31.7%〜40.2%、
七句去物導入前を上回る）。ng内訳を集計した結果、違反エントリ118件中74件
（63%）が七句去物によるもので突出して最大の要因。**七句去物実装は成功であり、
ng率54%はバグではなく新ルールが設計通りに発火した結果である。**

**判明した偏り：** 七句去物ヒット74件のうち「夢」47件・「月」22件の2語だけで
約93%（69/74相当）を占める。竹・船・松・煙は今回のrunで一度も違反要因に
ならなかった。

**其の五十八Phase 0検算とのギャップ：** Phase 0（湯山三吟・遺誡百韻・水無瀬三吟
3作品30ペア、違反0件）は「完成された古典作品では7句間隔が自然に守られている」
ことを示したものであり、「生成時の候補プールがこの制約を苦もなく満たせる」
ことは意味しなかった。実際にはLLM（メンタムさん）が付句に窮した際に
「夢」「月」のような当たり障りのない語彙に逃げ込む傾向があり、これが7句間隔
制約と衝突して却下率を押し上げた。Phase 0の実作品検算と実際の生成頻度には
大きなギャップがあり、これは今後同種の去嫌ルールを追加する際にも留意すべき
一般的な教訓である。

**意義：** 「夢」「月」への収束は、課題一覧#3（シード辞書の秋偏重・除外後
プールの雑優勢）と根が同じ現象と考えられ、「雑」の内部構造が未分化（恋・
旅・述懐等の部立ヒントがない）なために季節から外れた場面で生成が同じ狭い
語彙に収束しやすいという仮説を補強する実証データとなった。今後予定している
「雑の構造化」（恋・旅・述懐の部立ヒント新設）の優先度を裏付ける根拠として
位置づける。

**詳細（内訳集計・語別カウント）は`docs/handover_20260724_其の六十一.md`参照。**
...
### D-58-1　七句去物ルールの実証検証：3作品とも違反0件、Phase2実装は見送り（別バグ発見のため）

**判断：** 水無瀬三吟・湯山三吟・遺誡百韻の3作品で七句去物10語（衣・竹・田・
船・夢・涙・月・松・枕・煙）の再出現間隔を検算した結果、**違反0件**
（水無瀬三吟の1件のみ「月日」の適用除外規定に該当し、むしろ規定遵守の
証拠）。ShikimokuChecker実装に進む価値ありと判断したが、実装ドラフト設計中に
無関係の重大な既存バグ（下記）を発見したため、**Phase 2は実ファイルへの
適用を見送り、ドラフト提示のみに留めた**（人間の判断）。

**発見した既存バグ（別件・要優先対応）：** `ShikimokuChecker#ichiza_violations`
（一座一句物チェック）は`verse[:word]`への部分文字列マッチで実装されているが、
`RengasController#create`・`build_verse_history`・
`script/observe_production_hyakuin.rb`のいずれも`history`/`candidate`に
`:word`キーを設定していない。そのため`ichiza_violations`の照合は常に
空文字列同士の比較になり、**本番・観測run双方で一座一句物の重複が実質
検出できないまま運用されている。** 七句去物を同じ`:word`部分一致方式で
実装しても同じ配線ギャップを継承するため、テストは通っても本番では
発火しない「見せかけの実装」になる。

**結論：** 七句去物のドラフト（`app/data/nanaku_gomono_words.yml`・
`ShikimokuChecker#nanaku_gomono_violations`・`verify_shikimoku.rb`試験14）は
`docs/handover_20260724_其の五十八.md`に全文記録した。次回、ichiza配線
ギャップ（`RengasController`のcandidate/historyに`word:`を追加するだけで
解消できる見込み）を修正した上で、七句去物ドラフトもあわせて適用することを
推奨する（D-33-1の承認ゲート対象）。

**検証データの詳細（作品別出現句番号・間隔一覧）は`docs/handover_20260724_
其の五十八.md`のPhase 0セクションを参照。**

---

### D-58-2　未登録語彙8語をBuiDictionaryへ追加

**判断：** 其の五十七で判明した未登録語彙（月・船・夢・枕・涙・煙）と
春季語補強候補（いとゆふ・帰雁）を`app/data/bui_dictionary.yml`に追加した
（コミット`71b73ac`）。「月」はKIGO_BUI（renga_generator.rb）には既に
光物として登録済みだったがBuiDictionary.yml側は未登録という非対称が
其の五十七で発見されており、本追加で解消した。

**primary_bui: null で登録した3語（夢・枕・涙）の効果について：**
`BuiDictionary#detect_all`は`primary_bui`が真値のときのみ結果に加えるため、
`primary_bui: null`での登録は`detect_all`の挙動を一切変えない
（未登録のままと機能的に同一）。この3語は「七句去物の構成語だが正規の
部立カテゴリを持たない」という情報を記録する目的のドキュメンテーション
追加であり、生成句の部立検出ロジックに影響しない。

**未検証の留保：** 「いとゆふ」「帰雁」がMeCab IPADIC標準辞書で1形態素として
正しく切り出されるかは未確認（D-22-2と同種の既知の限定）。次回Rails runner
環境での確認を推奨する。
...
**追記（同日中）：** 参照論文（勢田勝郭「連歌去嫌の総合的再検討」研究紀要52号）を
ユーザーが追加入手し（`docs/reference/h28kiyo7.pdf`、著作権配慮によりgit管理対象外）、
去嫌一覧表の列見出し・記号の意味が判明した。

- **「七」列＝七句去物：** 通常の部立とは別枠の去嫌規定。「衣・竹・田・船・夢・涙・
  月・松・枕」の9語（＋後代追加の「煙」で計10語）を含む句同士は7句以上間隔を
  空けなければならない（通常の部立の5句去・3句去より厳しい）。月は光物用法限定、
  松は植物用法限定など語ごとに適用条件あり
- **記号の意味：** ×＝通常はその部立に分類される語だが句の仕立てにより除外と
  判断／□＝当該部立語を含み直近の同部立語との間隔がちょうど規定の最小値（際どい
  ボーダーケース）／△▽＝朝夕時分の際どい間隔（二句）でも嫌われないことを示す注記。
  いずれも「違反」ではなく境界事例の注記。◎のみ意味未確定のまま残る

BuiDictionary実装時の注意点：七句去物（月・松・夢・枕・涙・船・煙）は通常の
部立とは異なる「7句以内再出現禁止」という制約を持つため、既存のShikimokuChecker/
BuiDictionaryの5句去・3句去の粒度では表現できない。実装するなら新規の制約軸が必要。
